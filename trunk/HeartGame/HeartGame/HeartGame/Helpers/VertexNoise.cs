...
namespace HeartGame
{
    class ColorNoise
    {
        static Perlin VertexNoiseX = new Perlin(RandomHelper.random.Next());
        static Perlin VertexNoiseY = new Perlin(RandomHelper.random.Next());
        static Perlin VertexNoiseZ = new Perlin(RandomHelper.random.Next());
        static Perlin GlobalVertexNoiseX = new Perlin(RandomHelper.random.Next());
        static Perlin GlobalVertexNoiseY = new Perlin(RandomHelper.random.Next());
        static Perlin GlobalVertexNoiseZ = new Perlin(RandomHelper.random.Next());

        static float NoiseScale = 0.4f;
        static float NoiseMagnitude = 10.0f;
        static float GlobalNoiseScale = 0.003f;
        static float GlobalNoiseMagnitude = 0.4f;
    ...
    class VertexNoise
    {
        ...
        static Perlin GlobalVertexNoiseY = new Perlin(RandomHelper.random.Next());
        static Perlin GlobalVertexNoiseZ = new Perlin(RandomHelper.random.Next());
        static float NoiseScale = 0.8f;
        static float NoiseMagnitude = 0.1f;
        static float GlobalNoiseScale = 0.00f;
        static float GlobalNoiseMagnitude = 0.0f;
        ...
        {
            float x = VertexNoiseX.Noise((float)position.X * NoiseScale, (float)position.Y * NoiseScale, (float)position.Z * NoiseScale) * NoiseMagnitude * 2.0f - NoiseMagnitude / 2.0f;
            float y = VertexNoiseY.Noise((float)position.X * NoiseScale, (float)position.Y * NoiseScale, (float)position.Z * NoiseScale) * NoiseMagnitude * 2.0f - NoiseMagnitude / 2.0f;
            float z = VertexNoiseZ.Noise((float)position.X * NoiseScale, (float)position.Y * NoiseScale, (float)position.Z * NoiseScale) * NoiseMagnitude * 2.0f - NoiseMagnitude / 2.0f;
